...
def main() -> int:
    ...
    if "temp_audio = str(" not in core_source or "os.replace(temp_audio, final_file)" not in core_source:
        errors.append("Export audio belum transaksional terhadap file parsial")
    if 'temp_token = f"{session_id}-{os.getpid()}"' not in core_source:
        errors.append("Temporary export file belum unik per session/process")
    if "temp_vtt_path = Path(final_vtt)" not in core_source or "os.replace(temp_vtt, final_vtt)" not in core_source:
        errors.append("Export VTT belum transaksional")
    ...
    aac_branch = core_source.split("elif session['output_format'] == 'aac':", 1)[1].split("elif session['output_format'] == 'flac':", 1)[0] if "elif session['output_format'] == 'aac':" in core_source else ""
    if "'-write_id3v2', '1'" not in aac_branch or "'-map_metadata', '1'" not in aac_branch:
        errors.append("AAC ADTS belum menulis metadata sebagai ID3v2")
    ...
    if 'data.get("version") != PRONUNCIATION_VERSION' not in pronunciation_source:
        errors.append("Cache pronunciation lama belum di-invalidasi berdasarkan versi")
